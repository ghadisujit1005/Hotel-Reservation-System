...
CREATE TABLE ROOMS(
    ...
    INSERT INTO CUSTOMERS VALUES (207,'SAHIL JADHAV','SAHILJADHAV@GMAIL.COM','8543542342','VIRVANI GOREGAON EAST');
    ...
    (303,203,103,'2025-08-18','92025-08-19','CANCELLED'),
    ...
    SELECT H.HOTEL_NAME, R.ROOM_NUMBER,ROOM_TYPE, R.AVAILABILITY FROM ROOMS R
    LEFT JOIN HOTELS H
    ON R.HOTEL_ID = H.HOTEL_ID
    WHERE R.AVAILABILITY ='YES';
    
    -- Q.2 LIST ALL ROOMS IN SAI DARBAR HOTEL.
    SELECT R.ROOM_ID, H.HOTEL_NAME, R.ROOM_NUMBER, R.ROOM_TYPE, R.PRICE_PER_NIGHT FROM  ROOMS R
	RIGHT JOIN HOTELS H
    ON R.HOTEL_ID = H.HOTEL_ID
    WHERE HOTEL_NAME = 'SAI DARBAR';
	...
	SELECT R.ROOM_ID,R.ROOM_NUMBER, R.ROOM_TYPE,R.PRICE_PER_NIGHT,R.AVAILABILITY FROM ROOMS R 
	JOIN HOTELS H
	ON R.HOTEL_ID = H.HOTEL_ID
	WHERE H.HOTEL_NAME = 'SHOURYWADA';
...
-- Q.12 GET ALL RESERVATIONS WHERE CHECK_OUT_DATE AFTER 2025-08-08.
	...
	ON R.CUSTOMER_ID = C.CUSTOMER_ID
	WHERE R.CHECK_OUT_DATE >'2025-08-08';
...
-- Q.19 LIST RESARVATION WITH CUSTOMER AND HOTEL DEITAILS. 
	SELECT H.HOTEL_ID, H.HOTEL_NAME, H.LOCATION, H.RATING, C.CUSTOMER_ID, C.FULL_NAME, R.STATUS FROM RESERVATION R 
	JOIN CUSTOMERS C 
	ON R.CUSTOMER_ID = C.CUSTOMER_ID
	JOIN ROOMS RO
	ON RO.ROOM_ID = R.ROOM_ID
...
-- Q.20 SHOW PAYMENT WITH CUSTOMER 'SHAMIKA SINGH' OR HOTELS NAME 'NIKHIL'. 
SELECT  C.FULL_NAME,  H.HOTEL_NAME, P.PAYMENT_METHOD FROM HOTELS H 
JOIN  ROOMS RO
ON H.HOTEL_ID = RO.HOTEL_ID
JOIN RESERVATION R 
ON R.ROOM_ID = RO.ROOM_ID
JOIN PAYMENTS P 
ON P.RESERVATION_ID = R.RESERVATION_ID
JOIN CUSTOMERS C 
ON C.CUSTOMER_ID = R.CUSTOMER_ID
WHERE C.FULL_NAME ='SHAMIKA SINGH' OR H.HOTEL_NAME = 'NIKHIL';


-- Q.21 FIND RESERVATION WITH THEIR ASSIGNED STAFF (RECEPTIONIST).
	SELECT S.FULL_NAME, H.HOTEL_NAME, S.ROLE, R.STATUS FROM STAFF S
	JOIN HOTELS H
	ON S.HOTEL_ID = H.HOTEL_ID
	JOIN ROOMS RO
	ON H.HOTEL_ID = RO.HOTEL_ID
...
-- Q.23 FIND HOTEL WITH MAXIMUM TOTAL REVENUE.
SELECT H.HOTEL_ID,H.HOTEL_NAME,SUM(P.AMOUNT) AS MAXIMUM_TOTAL_REVENUE FROM PAYMENTS P
JOIN RESERVATION R
ON P.RESERVATION_ID = R.RESERVATION_ID
JOIN ROOMS RO
ON R.ROOM_ID = RO.ROOM_ID
...
ORDER BY MAXIMUM_TOTAL_REVENUE DESC
LIMIT 1;

-- Q.24 COUNT HOW MANY CUSTOMERS BOOKED EACH ROOM TYPE.
SELECT R.ROOM_TYPE,COUNT(DISTINCT C.CUSTOMER_ID) AS TOTAL_CUSTOMERS 
FROM ROOMS R
JOIN RESERVATION RO 
ON R.ROOM_ID = RO.ROOM_ID
JOIN CUSTOMERS C 
...
-- (SUBQUERY)
-- Q.25 LIST CUSTOMERS WHO PAID MORE THAN THE AVERAGE PAYMENT.
...
-- Q.26 FIND CUSTOMERS WITH PAYMENTS GREATER THAN ALL OTHERS.
SELECT C.CUSTOMER_ID,C.FULL_NAME,P.AMOUNT FROM PAYMENTS P
JOIN RESERVATION R 
ON P.RESERVATION_ID = R.RESERVATION_ID
JOIN CUSTOMERS C 
ON C.CUSTOMER_ID = R.CUSTOMER_ID
WHERE P.AMOUNT > ALL (SELECT P2.AMOUNT FROM PAYMENTS P2 WHERE R.CUSTOMER_ID <> C.CUSTOMER_ID);

-- Q.27 FIND THE SECOND HIGHETS TOTAL PAYMENT BY CUSTOMER. 
SELECT CUSTOMER_ID, FULL_NAME, TOTAL_PAYMENT FROM
(SELECT C.CUSTOMER_ID,C.FULL_NAME,SUM(P.AMOUNT) AS TOTAL_PAYMENT
    FROM CUSTOMERS C
    JOIN RESERVATION R 
    ON C.CUSTOMER_ID = R.CUSTOMER_ID 
    JOIN PAYMENTS P 
    ...
    ORDER BY TOTAL_PAYMENT DESC LIMIT 2)AS T 
    ORDER BY TOTAL_PAYMENT ASC
    LIMIT 1;
    
    
-- Q.28 FIND STAFF IN HOTELS WHERE ALL ROOMS ARE AVAILABLE. 
SELECT S.STAFF_ID, S.FULL_NAME, H.HOTEL_NAME FROM STAFF S
JOIN HOTELS H ON S.HOTEL_ID = H.HOTEL_ID
WHERE NOT EXISTS (SELECT 1 FROM ROOMS R WHERE R.HOTEL_ID = H.HOTEL_ID AND R.AVAILABILITY = 'YES');


-- Q.29 FIND RESERVATION WITH PAYMENT HIGHER THAN HOTEL'S AVERAGE ROOM PRICE.
SELECT R.RESERVATION_ID,C.CUSTOMER_ID,C.FULL_NAME,H.HOTEL_NAME,P.AMOUNT AS PAYMENT,RO.PRICE_PER_NIGHT AS ROOM_PRICE
FROM RESERVATION R
JOIN CUSTOMERS C ON R.CUSTOMER_ID = C.CUSTOMER_ID
JOIN ROOMS RO ON R.ROOM_ID = RO.ROOM_ID
JOIN HOTELS H ON RO.HOTEL_ID = H.HOTEL_ID
JOIN PAYMENTS P ON R.RESERVATION_ID = P.RESERVATION_ID
WHERE P.AMOUNT > (SELECT AVG(RO2.PRICE_PER_NIGHT) FROM ROOMS RO2 WHERE RO2.HOTEL_ID = H.HOTEL_ID);

-- Q.30 GET STAFF WHO WORK IN HOTELS WITH HIGHEST RATING.
